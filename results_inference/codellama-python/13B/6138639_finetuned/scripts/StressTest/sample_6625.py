...
if sheep_to_horses_ratio_hypothesis <= sheep_to_horses_ratio_premise:
    # check if the hypothesis value contradicts the estimate of more than'sheep_to_horses_ratio_premise'
    label = "contradiction"
else:
    # the premise gives only an estimate for the ratio between the number of sheep and the number of horses
    # any ratio greater than'sheep_to_horses_ratio_premise' is consistent with the premise, but cannot be explicitly entailed from the premise
    label = "neutral"
