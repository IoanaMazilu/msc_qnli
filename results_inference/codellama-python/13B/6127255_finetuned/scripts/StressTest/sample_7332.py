samsung_phone_premise = 75
samsung_phone_hypothesis = 25
iphone_user_premise = 70
iphone_user_hypothesis = 70
samsung_company_premise = 85
samsung_company_hypothesis = 85

# the hypothesis refers to the percentage of Samsung phone and iPhone users in ABC limited and the same company, mentioned in the premise
if samsung_phone_premise <= samsung_phone_hypothesis:
    # check if the estimate of'samsung_phone_hypothesis' contradicts the percentage of Samsung phone users in the premise
    label = "contradiction"
elif iphone_user_hypothesis!= iphone_user_premise:
    # check if the percentage of iPhone users in the hypothesis contradicts the percentage of iPhone users reported in the premise
    label = "contradiction"
elif samsung_company_hypothesis!= samsung_company_premise:
    # check if the percentage of Samsung phone users in the same company in the hypothesis contradicts the percentage of Samsung phone users in the same company reported in the premise
    label = "contradiction"
else:
    # if the hypothesis values and estimates do not contradict the premise ones, we can infer entailment
    label = "entailment"

print(label)
