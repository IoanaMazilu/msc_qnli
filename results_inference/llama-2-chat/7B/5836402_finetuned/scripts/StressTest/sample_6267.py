death_rate_premise = 0.10
death_rate_hypothesis = 0.30
fear_rate_premise = 0.20
fear_rate_hypothesis = 0.20

# the hypothesis refers to the death rate and fear rate of a village in Sri Lanka mentioned in the premise
if death_rate_hypothesis >= death_rate_premise:
    # check if the estimate of 'death_rate_hypothesis' contradicts the death rate in the premise
    label = "contradiction"
elif fear_rate_hypothesis!= fear_rate_premise:
    # check if the fear rate in the hypothesis contradicts the fear rate reported in the premise
    label = "contradiction"
else:
    # if the hypothesis values and estimates do not contradict the premise ones, we can infer entailment
    label = "entailment"

print(label)
