ysulp-etal:2020:1
---

# Premise
geese_premise = 58.0
ducks_premise = 37.0

# Hypothesis
more_geese_hypothesis = 21.0

# The hypothesis refers to the number of geese, which is also mentioned in the premise
# Compute the total number of geese in the premise
total_geese_premise = geese_premise + more_geese_hypothesis
if total_geese_premise!= geese_premise:
    # Check if the total number of geese in the hypothesis contradicts the number of geese in the premise
    label = "contradiction"
else:
    # If the hypothesis values and estimates do not contradict the premise values, we can infer entailment
    label = "entailment"

print(label)
