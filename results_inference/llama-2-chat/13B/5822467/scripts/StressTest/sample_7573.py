fred_speed_premise = 7
fred_speed_hypothesis = 2
sam_speed_premise = 5

# the hypothesis refers to the speed of Fred and Sam, mentioned in the premise
if fred_speed_premise > fred_speed_hypothesis and sam_speed_premise > fred_speed_hypothesis:
    # check if the estimate of 'fred_speed_hypothesis' contradicts the speed of Fred in the premise
    # check if the estimate of'sam_speed_premise' contradicts the speed of Sam in the premise
    label = "contradiction"
elif fred_speed_hypothesis!= sam_speed_premise:
    # check if the speed of Sam in the hypothesis contradicts the speed of Sam reported in the premise
    label = "contradiction"
else:
    # if the hypothesis values and estimates do not contradict the premise ones, we can infer entailment
    label = "entailment"

print(label)
