susan_weight_premise = 40
susan_weight_hypothesis = 10
total_weight_premise = 110
total_weight_hypothesis = 110

# the hypothesis talks about the weight of Susan and Anna, referenced also in the premise
if susan_weight_hypothesis + susan_weight_premise > total_weight_premise:
    # check if the weight of Susan in the hypothesis contradicts the total weight of the two people in the premise
    label = "contradiction"
elif total_weight_hypothesis!= total_weight_premise:
    # check if the total weight of the two people in the hypothesis contradicts the total weight of the two people reported in the premise
    label = "contradiction"
else:
    # if the hypothesis values and estimates do not contradict the premise ones, we can infer entailment
    label = "entailment"

print(label)
