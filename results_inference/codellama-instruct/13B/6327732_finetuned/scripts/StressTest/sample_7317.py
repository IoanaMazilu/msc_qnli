...
if time_jim_stretches_hypothesis <= time_jim_stretches_premise:
    # check if the hypothesis value contradicts the estimate of 'time_jim_stretches_premise'
    label = "contradiction"
else:
    # the premise gives only an estimate for the time it takes Jim to stretch
    # any time it takes Jim to stretch greater than 'time_jim_stretches_premise' is consistent with the premise, but cannot be explicitly entailed from the premise
    label = "neutral"
