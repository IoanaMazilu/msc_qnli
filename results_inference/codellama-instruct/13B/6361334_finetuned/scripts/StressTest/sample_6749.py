...
if butter_mixed_hypothesis <= butter_mixed_premise:
    # check if the estimate of 'butter_mixed_hypothesis' contradicts the quantity of butter mixed in the premise
    label = "contradiction"
else:
    # the premise gives only an estimate for the quantity of butter mixed
    # any quantity of butter greater than 'butter_mixed_premise' is consistent with the premise, but cannot be explicitly entailed from the premise
    label = "neutral"
